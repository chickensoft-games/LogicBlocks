...
public static class LogicBlockTypes {
  ...
  public static IDictionary<Type, Attribute[]> GetAttributes(
    ITypeRegistry registry, Type type
  ) {
    ...
    do {
      if (registry.Metatypes.TryGetValue(currentType, out var metatype)) {
        foreach (var attrType in metatype.Attributes.Keys) {
          attributes.Add(attrType, metatype.Attributes[attrType]);
        }
      }
  ...
  public static JsonTypeInfo CreateJsonTypeInfo(
    ...
    // Register derived types by looking at the generated type registry.
